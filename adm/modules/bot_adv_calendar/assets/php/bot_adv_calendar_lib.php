 ...
 * ROLE: Бизнес-логика модуля bot_adv_calendar
 */
...
if (!function_exists('bot_adv_calendar_now')) {
  ...
  function bot_adv_calendar_require_schema(PDO $pdo): void
  {
    ...
    if ($missing) {
      throw new RuntimeException(
        'Не применен SQL модуля bot_adv_calendar. Отсутствуют таблицы: ' . implode(', ', $missing)
      );
    }
  ...
  {
    if ($weekday === 0) return 'Каждый день';
    $map = bot_adv_calendar_weekday_map();
    return (string)($map[$weekday] ?? ('День #' . $weekday));
  }
  ...
  function bot_adv_calendar_user_options_save(PDO $pdo, int $userId, array $input, int $updatedBy = 0): array
  {
    ...
    $endMinutes = (int)substr($workEnd, 0, 2) * 60 + (int)substr($workEnd, 3, 2);
    if ($endMinutes <= $startMinutes) {
      throw new RuntimeException('Окончание рабочего дня должно быть позже начала');
    }
  ...
  function bot_adv_calendar_user_window_add(PDO $pdo, int $userId, array $input): array
  {
    ...
    if ($timeFrom === null) {
      return ['ok' => false, 'error' => 'Укажите корректное время начала'];
    }
    ...
    if ($windowType === 'range') {
      ...
      if ($timeTo === null) {
        return ['ok' => false, 'error' => 'Укажите корректное время окончания'];
      }
      ...
      if ($toMinutes <= $fromMinutes) {
        return ['ok' => false, 'error' => 'Окончание окна должно быть позже начала'];
      }
    }
  ...
  function bot_adv_calendar_client_adv_slots_text(array $slots, int $limit = 12): string
  {
    ...
    if (!$slots) return 'Доступных рекламных окон нет.';
    ...
    foreach ($slots as $slot) {
      ...
      $line = '• ' . $time . ' ' . ($isBusy ? '+' : '-') . ' ' . $priceText;
      if ($userName !== '') $line .= ' (' . $userName . ')';
      $lines[] = $line;
      $n++;
    }

    if (!$lines) return 'Доступных рекламных окон нет.';
    if (!$showAll && $total > $n) {
      $lines[] = '… и еще ' . ($total - $n) . ' окон';
    }
  ...
  function bot_adv_calendar_client_day_slots_keyboard(string $date, array $slots): array
  {
    ...
    if (count($slots) > $shown) {
      $kb[] = [[
        'text' => '… еще ' . (count($slots) - $shown) . ' окон',
        'callback_data' => 'calendar_noop',
      ]];
    }

    $kb[] = [
      ['text' => 'Обновить день', 'callback_data' => 'calendar_day:' . $date],
      ['text' => 'Календарь', 'callback_data' => 'calendar_month:' . $month],
    ];
  ...
  function bot_adv_calendar_user_slots_text(array $slots, int $limit = 30): string
  {
    ...
    foreach ($slots as $slot) {
      ...
      if ($isBusy) {
        ...
        $label = ($busyName !== '') ? ('занято - ' . $busyName) : 'занято';
        $lines[] = $time . ' | ' . $label . $priceText;
      } else {
        $lines[] = $time . ' | свободно' . $priceText;
      }
      $i++;
    }

    if (count($slots) > $i) {
      $lines[] = '... еще ' . (count($slots) - $i) . ' окон';
    }
  ...
  function bot_adv_calendar_user_day_slots_keyboard(string $date, array $slots): array
  {
    ...
    if (count($freeSlots) > $shown) {
      $kb[] = [[
        'text' => '... еще ' . (count($freeSlots) - $shown) . ' окон',
        'callback_data' => 'calendar_noop',
      ]];
    }
  ...
  function bot_adv_calendar_user_calendar_text(string $monthStart): string
  {
    ...
    return "Календарь рекламы (мои окна): " . bot_adv_calendar_client_month_label($monthStart) . "\n"
      . "Выберите день.\n"
      . "{$g} свободно  {$y} мало мест  {$r} нет мест\n"
      . "Ячейка: цвет + номер дня.";
  }
  ...
  function bot_adv_calendar_process_user_booking_input(PDO $pdo, array $meta, array $link, array $settings): array
  {
    ...
    if (($token['ok'] ?? false) !== true) {
      if ($botToken !== '') {
        tg_send_message(
          $botToken,
          $chatId,
          'Ответьте на сообщение со слотом: @user_nameTG.'
        );
      }
    ...
    if ($date < date('Y-m-d')) {
      if ($botToken !== '') {
        tg_send_message($botToken, $chatId, 'Прошедшие даты недоступны для бронирования.');
      }
    ...
    if ($userId <= 0) {
      if ($botToken !== '') {
        tg_send_message($botToken, $chatId, 'Не удалось определить пользователя CRM для бронирования.');
      }
    ...
    if (!$slot) {
      if ($botToken !== '') {
        tg_send_message(
          $botToken,
          $chatId,
          "Слот {$time} на {$date} уже недоступен.\nОбновите день и выберите другой.",
    ...
    if ((int)($slot['is_busy'] ?? 0) === 1) {
      if ($botToken !== '') {
        tg_send_message(
          $botToken,
          $chatId,
          "Слот {$time} уже занят. Выберите другой.",
          ['reply_markup' => bot_adv_calendar_user_day_slots_keyboard($date, $slots)]
        );
      }
    ...
    try {
      ...
      if ($clientId <= 0) {
        throw new RuntimeException('Не удалось определить клиента.');
      }
      ...
      if ($botToken !== '') {
        tg_send_message($botToken, $chatId, 'Ошибка бронирования. Попробуйте снова.');
      }
  ...
  function bot_adv_calendar_create_adv_booking_request(PDO $pdo, int $clientId, string $date, array $slot): array
  {
    ...
    if ($clientName === '') $clientName = 'Клиент #' . $clientId;
    ...
    try {
      ...
      requests_add_comment($pdo, $requestId, null, $clientId, 'client', 'Бронь рекламы через Telegram-бот.');
  ...
  function bot_adv_calendar_generate_link_token(PDO $pdo, string $actorType, int $actorId, int $createdBy = 0): string
  {
    ...
    if (!bot_adv_calendar_is_link_token_actor_type($actorType)) {
      throw new RuntimeException('Некорректный actor_type');
    }
    if ($actorId <= 0) {
      throw new RuntimeException('Некорректный actor_id');
    }
    if ($actorType === 'user' && !bot_adv_calendar_user_is_attached($pdo, $actorId)) {
      throw new RuntimeException('Пользователь не подключен к bot_adv_calendar');
    }

    $actor = bot_adv_calendar_actor_row($pdo, $actorType, $actorId);
    if (!$actor) {
      throw new RuntimeException('Сущность для привязки не найдена');
    }
  ...
  function bot_adv_calendar_find_or_create_client(PDO $pdo, string $clientName, string $phoneNorm): array
  {
    ...
    try {
      ...
      // Вероятно гонка по unique(phone): пробуем повторно найти клиента.
  ...
  function bot_adv_calendar_bind_client_from_onboarding(PDO $pdo, array $chatMeta, string $clientName, string $phoneRaw): array
  {
    ...
    if (strlen($phoneNorm) !== 11 || $phoneNorm[0] !== '7') {
      return ['ok' => false, 'reason' => 'phone_invalid', 'message' => 'Телефон некорректен. Формат: 79XXXXXXXXX.'];
    }
    ...
    try {
      ...
      if (($link['ok'] ?? false) !== true) {
        throw new RuntimeException('Не удалось привязать Telegram чат.');
      }
      ...
      return [
        'ok' => true,
        'reason' => 'linked',
        'message' => 'Привязка клиента выполнена.',
        'actor_type' => 'client',
        'actor_id' => $clientId,
      ...
      return [
        'ok' => false,
        'reason' => 'db_error',
        'message' => 'Не удалось создать или привязать клиента.',
        'error' => $e->getMessage(),
      ];
    }
  }
  ...
  function bot_adv_calendar_bind_chat_by_start_token(PDO $pdo, string $rawToken, array $chatMeta): array
  {
    ...
    if (!is_array($tokenRow)) {
      return ['ok' => false, 'reason' => 'token_invalid', 'message' => 'Код недействителен или истек.'];
    }
    ...
    if (!bot_adv_calendar_is_link_token_actor_type($actorType)) {
      return ['ok' => false, 'reason' => 'token_forbidden_actor', 'message' => 'Коды привязки доступны только для площадок.'];
    }
    if ($actorType === 'user' && !bot_adv_calendar_user_is_attached($pdo, $actorId)) {
      return ['ok' => false, 'reason' => 'user_not_attached', 'message' => 'Пользователь CRM не подключен к bot_adv_calendar в админке.'];
    }

    $actor = bot_adv_calendar_actor_row($pdo, $actorType, $actorId);
    if (!$actor) {
      return ['ok' => false, 'reason' => 'actor_not_found', 'message' => 'Сущность привязки не найдена в CRM.'];
    }
    ...
    try {
      ...
      if (($link['ok'] ?? false) !== true) {
        throw new RuntimeException('Не удалось сохранить привязку чата.');
      }
      ...
      return [
        'ok' => false,
        'reason' => 'db_error',
        'message' => 'Ошибка привязки Telegram к сущности CRM.',
        'error' => $e->getMessage(),
      ];
    }
  ...
  {
    $label = ($actorType === 'client') ? 'клиента' : 'пользователя CRM';
    $text = "Меню {$label}. Выберите действие:";
  ...
  function bot_adv_calendar_client_calendar_text(string $monthStart): string
  {
    ...
    $y = bot_adv_calendar_utf8_icon('yellow');
    $r = bot_adv_calendar_utf8_icon('red');
    return "Календарь рекламы: " . bot_adv_calendar_client_month_label($monthStart) . "\n"
      . "Выберите день.\n"
      . "{$g} свободно  {$y} мало мест  {$r} нет мест\n"
      . "В ячейке: цвет + число.";
  }
  ...
  function bot_adv_calendar_process_callback(PDO $pdo, array $settings, array $meta): array
  {
    ...
    if ($callbackId !== '' && $botToken !== '') {
      tg_answer_callback_query($botToken, $callbackId, ['text' => 'Принято']);
    }
    ...
    if (!$link || (int)($link['is_active'] ?? 0) !== 1) {
      if ($botToken !== '') {
        tg_send_message($botToken, $chatId, 'Сначала пройдите привязку: пользователь CRM вводит код, клиент отправляет имя и телефон.');
      }
    ...
    if ($actorType === 'user' && strpos($callbackData, 'calendar_user_book:') === 0) {
      ...
      if (($parsed['ok'] ?? false) !== true) {
        if ($botToken !== '') tg_send_message($botToken, $chatId, 'Некорректный слот бронирования.');
      ...
      if ($day < date('Y-m-d')) {
        if ($botToken !== '') tg_send_message($botToken, $chatId, 'Прошедшие даты недоступны для бронирования.');
        return ['ok' => true, 'handled' => true, 'reason' => 'calendar_user_book_past'];
      }
      if ($userId <= 0) {
        if ($botToken !== '') tg_send_message($botToken, $chatId, 'Не удалось определить пользователя CRM.');
      ...
      if (!$slot) {
        if ($botToken !== '') {
          tg_send_message(
            $botToken,
            $chatId,
            "Слот {$time} на {$day} уже недоступен.\nОбновите день и выберите другой.",
      ...
      if ($botToken !== '') {
        ...
        $msg = "Слот {$dayLabel} {$time}.\n"
          . "Ответьте на это сообщение: @user_nameTG.\n"
    ...
    if ($actorType === 'client' && strpos($callbackData, 'calendar_book:') === 0) {
      ...
      if ($clientId <= 0) {
        if ($botToken !== '') tg_send_message($botToken, $chatId, 'Сначала завершите привязку клиента.');
      ...
      if (!$slot) {
        if ($botToken !== '') {
          $msg = "Окно {$time} на {$day} уже недоступно.\nОбновите день и выберите другое.";
          $opts = [];
      ...
      if ((int)($slot['is_busy'] ?? 0) === 1) {
        if ($botToken !== '') {
          tg_send_message($botToken, $chatId, "Окно {$time} уже занято. Выберите другое.", [
      ...
      if (($create['ok'] ?? false) !== true) {
        ...
        if ($botToken !== '') {
          $msg = ($reason === 'slot_taken')
            ? 'Это окно уже занято. Выберите другое.'
            : 'Не удалось создать бронь рекламы. Попробуйте снова.';
          $opts = [];
          if ($freeSlots) $opts['reply_markup'] = bot_adv_calendar_client_day_slots_keyboard($day, $freeSlots);
          tg_send_message($botToken, $chatId, $msg, $opts);
        }
    ...
    if ($actorType === 'user' && preg_match('~^calendar_day:(\d{4}-\d{2}-\d{2})$~', $callbackData, $m)) {
      ...
      if ($userId <= 0) {
        if ($botToken !== '') {
          tg_send_message($botToken, $chatId, 'Не удалось определить пользователя CRM.');
        }
      ...
      if ($botToken !== '') {
        ...
        $stateText = ($state === 'green') ? 'Свободно' : (($state === 'yellow') ? 'Мало мест' : 'Нет мест');
        $dayLabel = date('d.m.Y', strtotime($day));
        ...
        $msg = "Дата: {$dayLabel}\n"
          . "Статус: {$emoji} {$stateText}\n"
          . "Свободных окон: {$slotsTotal}\n"
          . "Занятых окон: {$busyTotal}"
          . $slotsListText
          . "\n\nВыберите свободное окно ниже.";
    ...
    if ($botToken !== '') {
      tg_send_message($botToken, $chatId, 'Команда пока не поддерживается.');
    }
  ...
  function bot_adv_calendar_process_update(PDO $pdo, array $settings, array $update): array
  {
    ...
    $isMenu = (bool)(preg_match('~^/(menu)$~iu', $text) || $textLower === 'меню');
    $isCalendar = (bool)(
      preg_match('~^/(calendar)$~iu', $text)
      || $textLower === 'календарь'
      || $textLower === 'календарь рекламы'
    );
    ...
    if ($bindToken !== '') {
      ...
      if (($bind['ok'] ?? false) === true) {
        if ($botToken !== '' && $chatId !== '') {
          ...
          $okText = $name !== '' ? ('Привязка выполнена: ' . $name . '.') : 'Привязка выполнена.';
      ...
      if ($botToken !== '' && $chatId !== '') {
        $errText = trim((string)($bind['message'] ?? 'Ошибка привязки.'));
    ...
    if ($isCancel) {
      ...
      if ($botToken !== '') {
        tg_send_message(
          $botToken,
          $chatId,
          "Регистрация сброшена.\nНапишите имя клиента.\n\nЕсли у вас есть код CRM, отправьте его одним сообщением.",
          ['reply_markup' => bot_adv_calendar_hide_menu_keyboard()]
        );
      }
    ...
    if (!$onboarding || $isStartWithoutToken || $isMenu || $forceOnboardingStart) {
      ...
      if ($botToken !== '') {
        tg_send_message(
          $botToken,
          $chatId,
          "Для клиента нужна быстрая регистрация.\n1) Напишите имя.\n2) Затем отправьте номер телефона.\n\nЕсли у вас есть код CRM, отправьте его одним сообщением.",
          ['reply_markup' => bot_adv_calendar_hide_menu_keyboard()]
        );
      }
    ...
    if ($step === 'await_name') {
      if (strpos($text, '/') === 0) {
        if ($botToken !== '') {
          tg_send_message($botToken, $chatId, 'Напишите имя клиента обычным текстом.', [
            'reply_markup' => bot_adv_calendar_hide_menu_keyboard(),
          ]);
        }
      ...
      if ($nameLen < 2) {
        if ($botToken !== '') {
          tg_send_message($botToken, $chatId, 'Имя слишком короткое. Введите имя клиента еще раз.', [
            'reply_markup' => bot_adv_calendar_hide_menu_keyboard(),
          ]);
        }
      ...
      if ($botToken !== '') {
        tg_send_message($botToken, $chatId, 'Отлично. Теперь отправьте номер телефона клиента (например, +7 999 123-45-67).', [
          'reply_markup' => bot_adv_calendar_hide_menu_keyboard(),
        ]);
      }
    ...
    if (strpos($text, '/') === 0) {
      if ($botToken !== '') {
        tg_send_message($botToken, $chatId, 'Ожидаю номер телефона клиента. Пример: +7 999 123-45-67.', [
          'reply_markup' => bot_adv_calendar_hide_menu_keyboard(),
        ]);
      }
    ...
    if ($botToken !== '') {
      if (($bindClient['ok'] ?? false) === true) {
        ...
        $okText = $created ? 'Клиент создан и привязан.' : 'Клиент найден и привязан.';
        if ($actorName !== '') {
          $okText .= ' ' . $actorName . '.';
        }
        tg_send_message($botToken, $chatId, $okText);
        ...
        $errText = trim((string)($bindClient['message'] ?? 'Ошибка привязки клиента.'));
        tg_send_message($botToken, $chatId, $errText . "\nОтправьте номер телефона еще раз.", [
          'reply_markup' => bot_adv_calendar_hide_menu_keyboard(),
        ]);
      }
    }
