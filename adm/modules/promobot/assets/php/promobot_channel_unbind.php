<?php
/**
 * FILE: /adm/modules/promobot/assets/php/promobot_channel_unbind.php
...
$pdo->prepare("DELETE FROM " . PROMOBOT_TABLE_CHANNELS . " WHERE id = :id AND bot_id = :bid LIMIT 1")
  ->execute([':id' => $channelId, ':bid' => $botId]);

$uid = (int)(function_exists('auth_user_id') ? auth_user_id() : 0);
$role = function_exists('auth_user_role') ? (string)auth_user_role() : '';

audit_log(PROMOBOT_MODULE_CODE, 'channel_unbind', 'info', [
  'channel_id' => $channelId,
], PROMOBOT_MODULE_CODE, null, $uid, $role);

flash(promobot_t('promobot.flash_channel_unbound'), 'ok');
