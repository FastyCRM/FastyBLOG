<?php
/**
 * FILE: /adm/modules/promobot/assets/php/promobot_channel_probe.php
 ...
 * @return bool
 */
function promobot_probe_tg_is_numeric_chat_id(string $chatId): bool
{
 ...
 */
function promobot_probe_max_get_json(string $url, array $headers = [], int $timeout = 20): array
{
  $url = trim($url);
 ...
 * @return array<string,mixed>
 */
function promobot_probe_max_pick_me(array $json): array
{
  $node = [];
 ...
 */
function promobot_probe_max_pick_chats(array $json): array
{
  $list = [];
...
try {
  ...
  $existingRows = promobot_channels_list($pdo, $botId);
  $existingByChatId = [];
  ...
  if ($platform === PROMOBOT_PLATFORM_MAX) {
    ...
    if ($apiKey === '') {
      ...
      $meResp = promobot_probe_max_get_json($baseUrl . '/me', $headers, 20);
      $me['http_code'] = (int)($meResp['http_code'] ?? 0);
      $me['ok'] = (($meResp['ok'] ?? false) === true) && ($me['http_code'] >= 200 && $me['http_code'] < 300);
      if ($me['ok']) {
        $me['bot'] = promobot_probe_max_pick_me((array)($meResp['json'] ?? []));
      } else {
      ...
      $chatsResp = promobot_probe_max_get_json($baseUrl . '/chats?count=100', $headers, 25);
      $chats['http_code'] = (int)($chatsResp['http_code'] ?? 0);
      $chats['ok'] = (($chatsResp['ok'] ?? false) === true) && ($chats['http_code'] >= 200 && $chats['http_code'] < 300);
      if ($chats['ok']) {
        $items = promobot_probe_max_pick_chats((array)($chatsResp['json'] ?? []));
    ...
    if ($token === '') {
      ...
      if (function_exists('tg_get_chat')) {
        ...
        foreach ($items as $idx => &$item) {
          ...
          if (!promobot_probe_tg_is_numeric_chat_id($cid)) {
            $item['status'] = 'known';
            continue;
          }
  ...
  audit_log(PROMOBOT_MODULE_CODE, 'channel_probe', $auditLevel, [
    'bot_id' => $botId,
    'platform' => $platform,
  ...
  ], PROMOBOT_MODULE_CODE, null, $uid, (string)(function_exists('auth_user_role') ? auth_user_role() : ''));

  json_ok([
    'checked_at' => promobot_now(),
    'bot_id' => $botId,
    'platform' => $platform,
    'token_present' => ($platform === PROMOBOT_PLATFORM_MAX)
  ...
  audit_log(PROMOBOT_MODULE_CODE, 'channel_probe', 'error', [
    'bot_id' => $botId,
    'error' => $e->getMessage(),
  ], PROMOBOT_MODULE_CODE, null, $uid, (string)(function_exists('auth_user_role') ? auth_user_role() : ''));
