<?php
/**
 * FILE: /adm/modules/tg_system_clients/assets/php/tg_system_clients_lib.php
...
if (!function_exists('tg_system_clients_now')) {

  /**
   * tg_system_clients_now()
   * Текущее серверное время в формате DATETIME.
   *
   * @return string
   */
  function tg_system_clients_now(): string
  {
    return date('Y-m-d H:i:s');
  }

  /**
   * tg_system_clients_is_manage_role()
   * Проверяет, может ли роль управлять модулем.
   *
   * @param array<int,string> $roles
   * @return bool
   */
  function tg_system_clients_is_manage_role(array $roles): bool
  {
    return in_array('admin', $roles, true) || in_array('manager', $roles, true);
  }

  /**
   * tg_system_clients_settings_defaults()
   * Возвращает дефолтные настройки модуля.
   *
   * @return array<string,mixed>
   */
  function tg_system_clients_settings_defaults(): array
  {
    return [
      'enabled' => 0,
      'bot_token' => '',
  ...
  /**
   * tg_system_clients_default_events()
   * Базовый список системных событий.
   *
   * @return array<int,array<string,mixed>>
   */
    function tg_system_clients_default_events(): array
  {
    return [
      [
        'event_code' => TG_SYSTEM_CLIENTS_EVENT_GENERAL,
        'title' => 'Общие уведомления',
        'description' => 'Служебные уведомления клиенту по работе CRM.',
        'sort_order' => 10,
      ],
      [
        'event_code' => TG_SYSTEM_CLIENTS_EVENT_REQUEST_ACCEPTED,
        'title' => 'Заявка принята',
        'description' => 'Уведомление клиенту о принятии заявки в работу.',
        'sort_order' => 20,
      ],
      [
        'event_code' => TG_SYSTEM_CLIENTS_EVENT_REQUEST_CONFIRMED,
        'title' => 'Заявка подтверждена',
        'description' => 'Уведомление клиенту о подтверждении заявки.',
        'sort_order' => 30,
      ],
      [
        'event_code' => TG_SYSTEM_CLIENTS_EVENT_REQUEST_CHANGED,
        'title' => 'Заявка изменена',
        'description' => 'Уведомление клиенту об изменении времени/специалиста/услуг.',
        'sort_order' => 40,
      ],
      [
        'event_code' => TG_SYSTEM_CLIENTS_EVENT_REQUEST_REMINDER_60M,
        'title' => 'Напоминание за 60 минут',
        'description' => 'Уведомление клиенту за час до визита.',
        'sort_order' => 50,
      ],
      [
        'event_code' => TG_SYSTEM_CLIENTS_EVENT_REQUEST_THANKS,
        'title' => 'Благодарность после визита',
        'description' => 'Сообщение клиенту после завершения заявки.',
        'sort_order' => 60,
      ],
      [
        'event_code' => TG_SYSTEM_CLIENTS_EVENT_PROMOTIONS,
        'title' => 'Акции и скидки',
        'description' => 'Рассылки по акциям, скидкам и программам лояльности.',
        'sort_order' => 70,
      ],
    ];
  }
  ...
  /**
   * tg_system_clients_schema_required_tables()
   * Возвращает список обязательных таблиц модуля.
   *
   * @return array<int,string>
   */
  function tg_system_clients_schema_required_tables(): array
  {
    return [
  ...
  /**
   * tg_system_clients_schema_db_name()
   * Возвращает имя текущей БД.
   *
   * @param PDO $pdo
   * @return string
   */
  function tg_system_clients_schema_db_name(PDO $pdo): string
  {
  ...
  /**
   * tg_system_clients_schema_missing_tables()
   * Возвращает список отсутствующих таблиц модуля.
   *
   * @param PDO $pdo
   * @return array<int,string>
   */
  function tg_system_clients_schema_missing_tables(PDO $pdo): array
  {
    $dbName = tg_system_clients_schema_db_name($pdo);
    if ($dbName === '') {
      return tg_system_clients_schema_required_tables();
    }

    $required = tg_system_clients_schema_required_tables();
  ...
  /**
   * tg_system_clients_require_schema()
   * Проверяет, что SQL модуля применён вручную.
   *
   * @param PDO $pdo
   * @return void
   */
  function tg_system_clients_require_schema(PDO $pdo): void
  {
    /**
    ...
    $missing = tg_system_clients_schema_missing_tables($pdo);
    if ($missing) {
      throw new RuntimeException(
        'Не применён SQL модуля tg_system_clients. Отсутствуют таблицы: ' . implode(', ', $missing)
      );
    }
  ...
  /**
   * tg_system_clients_settings_save()
   ...
   * @return array<string,mixed>
   */
  function tg_system_clients_settings_save(PDO $pdo, array $input): array
  {
    tg_system_clients_require_schema($pdo);
    ...
    $pdo->prepare("
      INSERT INTO " . TG_SYSTEM_CLIENTS_TABLE_SETTINGS . "
    ...
    return tg_system_clients_settings_get($pdo);
  }

  /**
   * tg_system_clients_events_get()
   * Возвращает список системных событий.
   *
   * @param PDO $pdo
   * @return array<int,array<string,mixed>>
   */
  function tg_system_clients_events_get(PDO $pdo): array
  {
    tg_system_clients_require_schema($pdo);

    $st = $pdo->query("
      SELECT id, event_code, title, description, global_enabled, sort_order
      FROM " . TG_SYSTEM_CLIENTS_TABLE_EVENTS . "
  ...
  /**
   * tg_system_clients_toggle_event()
   ...
   * @return bool
   */
  function tg_system_clients_toggle_event(PDO $pdo, string $eventCode, int $enabled): bool
  {
    tg_system_clients_require_schema($pdo);
    ...
    $st = $pdo->prepare("
      UPDATE " . TG_SYSTEM_CLIENTS_TABLE_EVENTS . "
  ...
  /**
   * tg_system_clients_user_preferences()
   ...
   * @return array<int,array<string,mixed>>
   */
  function tg_system_clients_user_preferences(PDO $pdo, int $clientId): array
  {
    tg_system_clients_require_schema($pdo);
    ...
    $st = $pdo->prepare("
      ...
      FROM " . TG_SYSTEM_CLIENTS_TABLE_EVENTS . " e
      LEFT JOIN " . TG_SYSTEM_CLIENTS_TABLE_CLIENT_EVENTS . " ue
        ON ue.event_code = e.event_code
       AND ue.client_id = :uid
      ORDER BY e.sort_order ASC, e.id ASC
    ");
    $st->execute([':uid' => $clientId]);
  ...
  /**
   * tg_system_clients_user_preferences_save()
   ...
   * @return void
   */
  function tg_system_clients_user_preferences_save(PDO $pdo, int $clientId, array $enabledMap): void
  {
    tg_system_clients_require_schema($pdo);
    if ($clientId <= 0) return;
    ...
    $st = $pdo->prepare("
      INSERT INTO " . TG_SYSTEM_CLIENTS_TABLE_CLIENT_EVENTS . "
      (client_id, event_code, enabled)
  ...
  /**
   * tg_system_clients_users_with_links()
   ...
   * @return array<int,array<string,mixed>>
   */
    function tg_system_clients_users_with_links(PDO $pdo, int $limit = 200, string $query = ''): array
  {
    tg_system_clients_require_schema($pdo);
    ...
    if ($query !== '') {
      ...
      $bindLike = static function (array &$partsRef, array &$paramsRef, string $expr, string $value, int &$idx): void {
        $name = ':q_like_' . $idx;
        $partsRef[] = $expr . ' LIKE ' . $name;
        $paramsRef[$name] = $value;
        $idx++;
      };

      $qLike = '%' . $query . '%';
      $bindLike($parts, $params, 'c.first_name', $qLike, $paramIndex);
      $bindLike($parts, $params, 'c.last_name', $qLike, $paramIndex);
      $bindLike($parts, $params, 'c.middle_name', $qLike, $paramIndex);
      $bindLike($parts, $params, "CONCAT_WS(' ', c.last_name, c.first_name, c.middle_name)", $qLike, $paramIndex);
      $bindLike($parts, $params, 'c.phone', $qLike, $paramIndex);
      $bindLike($parts, $params, 'CAST(c.inn AS CHAR)', $qLike, $paramIndex);

      $digits = preg_replace('/\D+/', '', $query) ?? '';
      if ($digits !== '' && strlen($digits) >= 4) {
        $digitsLike = $digits . '%';
        $bindLike($parts, $params, 'CAST(c.inn AS CHAR)', $digitsLike, $paramIndex);
        $bindLike($parts, $params, "REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(c.phone, ' ', ''), '-', ''), '(', ''), ')', ''), '+', ''), '.', '')", $digitsLike, $paramIndex);
      }
    ...
    $sql = "
      SELECT
        c.id,
        TRIM(CONCAT_WS(' ', c.last_name, c.first_name, c.middle_name)) AS name,
        c.first_name,
        c.last_name,
        c.middle_name,
        c.phone,
        ...
        l.chat_id,
        l.username,
        l.first_name AS tg_first_name,
        l.last_name AS tg_last_name,
        l.is_active,
        l.linked_at,
        l.last_seen_at
      FROM " . TG_SYSTEM_CLIENTS_CLIENTS_TABLE . " c
      LEFT JOIN " . TG_SYSTEM_CLIENTS_TABLE_LINKS . " l ON l.client_id = c.id
      " . $where . "
    ...
    foreach ($rows as &$row) {
      ...
      if ($name === '') {
        ...
        $row['name'] = ($fallback !== '' ? $fallback : ('Клиент #' . (int)($row['id'] ?? 0)));
      }
      $row['first_name'] = trim((string)($row['first_name'] ?? ''));
      $row['last_name'] = trim((string)($row['last_name'] ?? ''));
      $row['middle_name'] = trim((string)($row['middle_name'] ?? ''));
      $row['phone'] = trim((string)($row['phone'] ?? ''));
      $row['email'] = trim((string)($row['email'] ?? ''));
      $row['inn'] = trim((string)($row['inn'] ?? ''));
      $row['status'] = trim((string)($row['status'] ?? ''));
      $row['chat_id'] = trim((string)($row['chat_id'] ?? ''));
      $row['username'] = trim((string)($row['username'] ?? ''));
      $row['tg_first_name'] = trim((string)($row['tg_first_name'] ?? ''));
      $row['tg_last_name'] = trim((string)($row['tg_last_name'] ?? ''));
      $row['is_active'] = ((int)($row['is_active'] ?? 0) === 1) ? 1 : 0;
      $row['linked_at'] = trim((string)($row['linked_at'] ?? ''));
      $row['last_seen_at'] = trim((string)($row['last_seen_at'] ?? ''));
    }
    unset($row);
  ...
  /**
   * tg_system_clients_user_with_link_by_id()
   * Возвращает клиента и статус TG-привязки по точному client_id.
   *
   * @param PDO $pdo
   * @param int $clientId
   * @return array<string,mixed>|null
   */
  function tg_system_clients_user_with_link_by_id(PDO $pdo, int $clientId): ?array
  {
    tg_system_clients_require_schema($pdo);
   ...
   * @return array<string,mixed>
   */
  function tg_system_clients_unlink_user(PDO $pdo, int $clientId): array
  {
    tg_system_clients_require_schema($pdo);

    if ($clientId <= 0) {
      throw new RuntimeException('Некорректный client_id');
    }

    /**
     * $now — текущее серверное время.
     */
    $now = tg_system_clients_now();
    ...
    try {
      ...
      if (is_array($row)) {
        ...
        $pdo->prepare("
          UPDATE " . TG_SYSTEM_CLIENTS_TABLE_LINKS . "
          SET is_active = 0,
              last_seen_at = :now
          WHERE client_id = :client_id
          LIMIT 1
      ...
      $pdo->prepare("
        UPDATE " . TG_SYSTEM_CLIENTS_TABLE_LINK_TOKENS . "
        SET used_at = :used_at
        WHERE client_id = :client_id
          AND used_at IS NULL
      ")->execute([
  ...
  /**
   * tg_system_clients_generate_raw_token()
   * Генерирует короткий одноразовый код привязки (4 цифры).
   *
   * @return string
   */
  function tg_system_clients_generate_raw_token(): string
  {
    return (string)random_int(1000, 9999);
  }

  /**
   * tg_system_clients_generate_link_token()
   ...
   * @return string
   */
  function tg_system_clients_generate_link_token(PDO $pdo, int $clientId, int $createdBy = 0): string
  {
    tg_system_clients_require_schema($pdo);
    ...
    $stUser = $pdo->prepare("
      SELECT id
      FROM " . TG_SYSTEM_CLIENTS_CLIENTS_TABLE . "
      WHERE id = :id
      LIMIT 1
    ");
    $stUser->execute([':id' => $clientId]);
    ...
    while ($attempt < 50) {
      $candidate = tg_system_clients_generate_raw_token();
      $candidateHash = hash('sha256', $candidate);

      $stCheck = $pdo->prepare("
        SELECT id
        FROM " . TG_SYSTEM_CLIENTS_TABLE_LINK_TOKENS . "
    ...
    $pdo->prepare("
      INSERT INTO " . TG_SYSTEM_CLIENTS_TABLE_LINK_TOKENS . "
      (client_id, token_hash, expires_at, created_by)
      VALUES (:client_id, :token_hash, :expires_at, :created_by)
    ")->execute([
      ':client_id' => $clientId,
      ':token_hash' => $hash,
  ...
  /**
   * tg_system_clients_touch_link_by_chat()
   ...
   * @return void
   */
  function tg_system_clients_touch_link_by_chat(PDO $pdo, string $chatId): void
  {
    $chatId = trim($chatId);
    if ($chatId === '') return;
  ...
  /**
   * tg_system_clients_bind_chat_by_start_token()
   * Привязывает Telegram-чат к клиенту по коду привязки.
   *
   * @param PDO $pdo
   ...
   * @return array<string,mixed>
   */
  function tg_system_clients_bind_chat_by_start_token(PDO $pdo, string $rawToken, array $chatMeta): array
  {
    tg_system_clients_require_schema($pdo);
    ...
    $st = $pdo->prepare("
      SELECT t.id, t.client_id, t.expires_at, TRIM(CONCAT_WS(' ', u.last_name, u.first_name, u.middle_name)) AS name
      FROM " . TG_SYSTEM_CLIENTS_TABLE_LINK_TOKENS . " t
      JOIN " . TG_SYSTEM_CLIENTS_CLIENTS_TABLE . " u ON u.id = t.client_id
    ...
    try {
      ...
      $pdo->prepare("
        DELETE FROM " . TG_SYSTEM_CLIENTS_TABLE_LINKS . "
        WHERE chat_id = :chat_id
      ...
      $pdo->prepare("
        INSERT INTO " . TG_SYSTEM_CLIENTS_TABLE_LINKS . "
        (client_id, chat_id, chat_type, username, first_name, last_name, is_active, linked_at, last_seen_at)
        VALUES (:client_id, :chat_id, :chat_type, :username, :first_name, :last_name, 1, :linked_at, :last_seen_at)
        ON DUPLICATE KEY UPDATE
      ...
      return [
        'ok' => false,
        'reason' => 'db_error',
        'message' => 'Ошибка привязки Telegram к клиенту.',
        'error' => $e->getMessage(),
      ];
    }
   ...
   * @return void
   */
  function tg_system_clients_cleanup(PDO $pdo, int $retentionDays): void
  {
    if ($retentionDays < 1) $retentionDays = 7;
    if ($retentionDays > 30) $retentionDays = 30;

    $pdo->exec("
      DELETE FROM " . TG_SYSTEM_CLIENTS_TABLE_DISPATCH_LOG . "
      WHERE created_at < DATE_SUB(NOW(), INTERVAL " . (int)$retentionDays . " DAY)
    ");

    $pdo->exec("
      DELETE FROM " . TG_SYSTEM_CLIENTS_TABLE_LINK_TOKENS . "
   ...
   * @return array<string,mixed>
   */
  function tg_system_clients_send_system(PDO $pdo, string $message, string $eventCode = TG_SYSTEM_CLIENTS_EVENT_GENERAL, array $options = []): array
  {
    tg_system_clients_require_schema($pdo);
    ...
    $clientIds = [];
    if (isset($options['client_ids']) && is_array($options['client_ids'])) {
      foreach ($options['client_ids'] as $uid) {
        $uid = (int)$uid;
        if ($uid > 0) $clientIds[] = $uid;
      }
    ...
    $sql = "
      SELECT
        l.client_id,
        l.chat_id
      FROM " . TG_SYSTEM_CLIENTS_TABLE_LINKS . " l
      JOIN " . TG_SYSTEM_CLIENTS_CLIENTS_TABLE . " u ON u.id = l.client_id
      LEFT JOIN " . TG_SYSTEM_CLIENTS_TABLE_CLIENT_EVENTS . " ue
        ON ue.client_id = l.client_id
    ...
    if ($clientIds) {
      $in = [];
      foreach ($clientIds as $i => $uid) {
        $ph = ':u' . $i;
        $in[] = $ph;
        $params[$ph] = $uid;
      }
      $sql .= " AND l.client_id IN (" . implode(',', $in) . ")";
    }
    ...
    tg_system_clients_cleanup($pdo, $retentionDays);

    $sent = 0;
    $failed = 0;

    $stLog = $pdo->prepare("
      INSERT INTO " . TG_SYSTEM_CLIENTS_TABLE_DISPATCH_LOG . "
      (event_code, client_id, chat_id, message_text, send_status, error_text)
      VALUES (:event_code, :client_id, :chat_id, :message_text, :send_status, :error_text)
    ");

    foreach ($targets as $target) {
      $uid = (int)($target['client_id'] ?? 0);
  ...
  /**
   * tg_system_clients_apply_webhook()
   * Устанавливает webhook для бота модуля.
   *
   * @param array<string,mixed> $settings
   * @return array<string,mixed>
   */
  function tg_system_clients_apply_webhook(array $settings): array
  {
  ...
  /**
   * tg_system_clients_extract_message_data()
   * Извлекает из update минимальные поля для обработки.
   *
   * @param array<string,mixed> $update
   * @return array<string,mixed>
   */
  function tg_system_clients_extract_message_data(array $update): array
  {
  ...
  /**
   * tg_system_clients_process_update()
   ...
   * @return array<string,mixed>
   */
  function tg_system_clients_process_update(PDO $pdo, array $settings, array $update): array
  {
    $meta = tg_system_clients_extract_message_data($update);
    $chatId = (string)($meta['chat_id'] ?? '');
    $text = (string)($meta['text'] ?? '');
    $token = trim((string)($settings['bot_token'] ?? ''));

    if ($chatId !== '') {
      tg_system_clients_touch_link_by_chat($pdo, $chatId);
    }
    ...
    $bind = tg_system_clients_bind_chat_by_start_token($pdo, $bindToken, $meta);
    if ($token !== '' && $chatId !== '') {
      if (($bind['ok'] ?? false) === true) {
        $userName = trim((string)($bind['client_name'] ?? ''));
        $textOk = ($userName !== '')
          ? ('Привязка выполнена. Клиент: ' . $userName . '.')
          : 'Привязка выполнена.';
  ...
  /**
   * tg_system_clients_webhook_process()
   * Полный процесс обработки webhook-запроса модуля.
   *
   * @param PDO $pdo
   * @return array<string,mixed>
   */
  function tg_system_clients_webhook_process(PDO $pdo): array
  {
    tg_system_clients_require_schema($pdo);
    ...
    $result = tg_system_clients_process_update($pdo, $settings, $update);
    $result['http'] = 200;
