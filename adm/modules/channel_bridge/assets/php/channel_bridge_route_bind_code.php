<?php
/**
 * FILE: /adm/modules/channel_bridge/assets/php/channel_bridge_route_bind_code.php
 * ROLE: do=route_bind_code — генерация одноразового кода автопривязки чата Telegram к маршруту.
 * FLOW:
 *  - принимает route_id и side (source/target);
 *  - создаёт новый одноразовый код;
 *  - показывает код через flash-сообщение в UI.
 */

declare(strict_types=1);

if (!defined('ROOT_PATH')) exit;

require_once __DIR__ . '/../../settings.php';
require_once __DIR__ . '/channel_bridge_lib.php';

acl_guard(module_allowed_roles(CHANNEL_BRIDGE_MODULE_CODE));

$uid = (int)(function_exists('auth_user_id') ? auth_user_id() : 0);
$roles = function_exists('auth_user_roles') ? (array)auth_user_roles($uid) : [];
if (!channel_bridge_can_manage($roles)) {
  flash(channel_bridge_t('channel_bridge.error_forbidden'), 'danger', 1);
  redirect_return('/adm/index.php?m=' . CHANNEL_BRIDGE_MODULE_CODE);
}

$method = strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'GET'));
if ($method !== 'POST') {
  http_405('Method Not Allowed');
}

csrf_check((string)($_POST['csrf'] ?? ''));

$routeId = (int)($_POST['id'] ?? 0);
$side = trim((string)($_POST['side'] ?? ''));

if ($routeId <= 0) {
  flash(channel_bridge_t('channel_bridge.error_bad_id'), 'danger', 1);
  redirect_return('/adm/index.php?m=' . CHANNEL_BRIDGE_MODULE_CODE);
}

try {
  $pdo = db();
  $code = channel_bridge_generate_bind_code($pdo, $routeId, $side, $uid);

  audit_log(CHANNEL_BRIDGE_MODULE_CODE, 'route_bind_code', 'info', [
    'route_id' => $routeId,
    'side' => $side,
  ], CHANNEL_BRIDGE_MODULE_CODE, null, $uid, (string)(function_exists('auth_user_role') ? auth_user_role() : ''));

  if ($side === CHANNEL_BRIDGE_BIND_SIDE_TARGET) {
    flash(channel_bridge_t('channel_bridge.flash_bind_code_target_created', ['code' => $code]), 'ok');
  } else {
    flash(channel_bridge_t('channel_bridge.flash_bind_code_source_created', ['code' => $code]), 'ok');
  }
} catch (Throwable $e) {
  audit_log(CHANNEL_BRIDGE_MODULE_CODE, 'route_bind_code', 'error', [
    'route_id' => $routeId,
    'side' => $side,
    'error' => $e->getMessage(),
  ], CHANNEL_BRIDGE_MODULE_CODE, null, $uid, (string)(function_exists('auth_user_role') ? auth_user_role() : ''));

  flash(channel_bridge_t('channel_bridge.flash_bind_code_generate_error', ['error' => $e->getMessage()]), 'danger', 1);
}

redirect_return('/adm/index.php?m=' . CHANNEL_BRIDGE_MODULE_CODE);

