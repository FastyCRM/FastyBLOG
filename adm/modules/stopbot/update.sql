...
SET @has_promo_source_col := (
  ...
  WHERE TABLE_SCHEMA = @db_name
    AND TABLE_NAME = 'stopbot_bots'
    AND COLUMN_NAME = 'promo_source_bot_id'
);

SET @sql_add_promo_source_col := IF(
  @has_promo_source_col = 0,
  'ALTER TABLE `stopbot_bots` ADD COLUMN `promo_source_bot_id` INT UNSIGNED NOT NULL DEFAULT 0 AFTER `max_send_path`',
  'SELECT 1'
);
...
SET @sql_add_promo_source_idx := IF(
  ...
  'ALTER TABLE `stopbot_bots` ADD KEY `idx_promo_source_bot_id` (`promo_source_bot_id`)',
  'SELECT 1'
);
