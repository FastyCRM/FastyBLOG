<?php
/**
 * FILE: /adm/modules/stopbot/assets/php/stopbot_user_detach.php
...
if ($userId <= 0 || $botId <= 0) {
  flash(stopbot_t('stopbot.flash_user_detach_fail'), 'danger', 1);
  redirect_return('/adm/index.php?m=' . STOPBOT_MODULE_CODE);
}

$pdo = db();
$pdo->prepare("DELETE FROM " . STOPBOT_TABLE_USER_ACCESS . " WHERE user_id = :uid AND bot_id = :bid")
  ->execute([':uid' => $userId, ':bid' => $botId]);

$uid = (int)(function_exists('auth_user_id') ? auth_user_id() : 0);
$role = function_exists('auth_user_role') ? (string)auth_user_role() : '';

audit_log(STOPBOT_MODULE_CODE, 'user_detach', 'info', [
  'user_id' => $userId,
  'bot_id' => $botId,
], STOPBOT_MODULE_CODE, null, $uid, $role);

flash(stopbot_t('stopbot.flash_user_detached'), 'ok');
