<?php
/**
 * FILE: /adm/modules/stopbot/assets/php/stopbot_promo_add.php
 * ROLE: do=promo_add — добавление промокода.
 */

declare(strict_types=1);

if (!defined('ROOT_PATH')) exit;

require_once __DIR__ . '/../../settings.php';
require_once __DIR__ . '/stopbot_lib.php';

acl_guard(module_allowed_roles(STOPBOT_MODULE_CODE));

$method = strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'GET'));
if ($method !== 'POST') {
  http_405('Method Not Allowed');
}

csrf_check((string)($_POST['csrf'] ?? ''));

$botId = (int)($_POST['bot_id'] ?? 0);
$keywords = trim((string)($_POST['keywords'] ?? ''));
$responseText = trim((string)($_POST['response_text'] ?? ''));
$isActive = ((int)($_POST['is_active'] ?? 0) === 1) ? 1 : 0;

$pdo = db();
$uid = (int)(function_exists('auth_user_id') ? auth_user_id() : 0);
$roles = function_exists('auth_user_roles') ? (array)auth_user_roles($uid) : [];

if (!stopbot_user_has_bot_access($pdo, $uid, $botId, $roles)) {
  flash(stopbot_t('stopbot.flash_access_denied'), 'danger', 1);
  redirect_return('/adm/index.php?m=' . STOPBOT_MODULE_CODE);
}

$promoOwnerBotId = stopbot_bot_promo_owner_id($pdo, $botId);
if ($promoOwnerBotId <= 0) {
  flash(stopbot_t('stopbot.flash_bot_not_found'), 'danger', 1);
  redirect_return('/adm/index.php?m=' . STOPBOT_MODULE_CODE);
}

if ($keywords === '') {
  flash(stopbot_t('stopbot.flash_promo_required'), 'danger', 1);
  redirect_return('/adm/index.php?m=' . STOPBOT_MODULE_CODE . '&bot_id=' . $botId);
}

$pdo->prepare("\n  INSERT INTO " . STOPBOT_TABLE_PROMOS . "
    (bot_id, keywords, response_text, is_active, created_by, updated_by)
  VALUES
    (:bot_id, :keywords, :response_text, :is_active, :created_by, :updated_by)
")->execute([
  ':bot_id' => $promoOwnerBotId,
  ':keywords' => $keywords,
  ':response_text' => $responseText,
  ':is_active' => $isActive,
  ':created_by' => $uid,
  ':updated_by' => $uid,
]);

$promoId = (int)$pdo->lastInsertId();
$role = function_exists('auth_user_role') ? (string)auth_user_role() : '';

audit_log(STOPBOT_MODULE_CODE, 'promo_add', 'info', [
  'promo_id' => $promoId,
  'bot_id' => $promoOwnerBotId,
  'context_bot_id' => $botId,
], STOPBOT_MODULE_CODE, null, $uid, $role);

flash(stopbot_t('stopbot.flash_promo_added'), 'ok');

redirect_return('/adm/index.php?m=' . STOPBOT_MODULE_CODE . '&bot_id=' . $botId);
