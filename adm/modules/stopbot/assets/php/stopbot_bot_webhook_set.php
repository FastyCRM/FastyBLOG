<?php
/**
 * FILE: /adm/modules/stopbot/assets/php/stopbot_bot_webhook_set.php
...
if ($token === '') {
  flash(stopbot_t('stopbot.flash_bot_token_empty'), 'danger', 1);
  redirect_return('/adm/index.php?m=' . STOPBOT_MODULE_CODE . '&bot_id=' . $botId);
}

$secret = trim((string)($bot['webhook_secret'] ?? ''));
$url = stopbot_bot_webhook_url($botId, STOPBOT_PLATFORM_TG, true);
...
if (($set['ok'] ?? false) === true) {
  ...
  audit_log(STOPBOT_MODULE_CODE, 'bot_webhook_set', 'info', [
    'bot_id' => $botId,
  ], STOPBOT_MODULE_CODE, null, $uid, $role);

  flash(stopbot_t('stopbot.flash_webhook_set_ok'), 'ok');
} else {
  audit_log(STOPBOT_MODULE_CODE, 'bot_webhook_set', 'warn', [
    'bot_id' => $botId,
    'error' => (string)($set['error'] ?? 'TG_SET_WEBHOOK_FAILED'),
  ], STOPBOT_MODULE_CODE, null, $uid, $role);

  flash(stopbot_t('stopbot.flash_webhook_set_fail'), 'danger', 1);
}
