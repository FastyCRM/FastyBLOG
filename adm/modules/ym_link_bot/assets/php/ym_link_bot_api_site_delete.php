...
$id = (int)($_POST['id'] ?? 0);
if ($id <= 0) json_err('site_id_required', 422);

try {
  $pdo = db();
  ymlb_sync_module_roles($pdo);
  if (!$canManage && !ymlb_site_is_owned_by_crm_user($pdo, $id, $uid)) {
    json_err('forbidden', 403);
  }
  ymlb_site_delete($pdo, $id);
  json_ok(['sites' => ymlb_sites_list_for_actor($pdo, $uid, $canManage)]);
} catch (Throwable $e) {
