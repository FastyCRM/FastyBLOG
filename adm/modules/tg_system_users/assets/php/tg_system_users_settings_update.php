<?php
/**
 * FILE: /adm/modules/tg_system_users/assets/php/tg_system_users_settings_update.php
 * ROLE: settings_update — сохранение настроек TG-бота
 */
...
if (!tg_system_users_is_manage_role($roles)) {
  audit_log('tg_system_users', 'settings_update', 'warn', [
...
/**
 * $input — значения настроек из формы.
 */
$input = [
...
/**
 * $applyWebhook — нужно ли применять webhook в Telegram.
 */
$applyWebhook = ((int)($_POST['apply_webhook'] ?? 0) === 1);

try {
  $saved = tg_system_users_settings_save($pdo, $input);

  audit_log('tg_system_users', 'settings_update', 'info', [
  ...
  if ($applyWebhook) {
    $res = tg_system_users_apply_webhook($saved);
  ...
  audit_log('tg_system_users', 'settings_update', 'error', [
    'error' => $e->getMessage(),
  ], 'module', null, $uid, $actorRole);
